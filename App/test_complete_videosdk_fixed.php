...
try {
    ...
    echo "<p class='success'>✅ API Key: " . substr($apiKey, 0, 8) . "...</p>";
    echo "<p class='success'>✅ Secret Key: " . substr($secretKey, 0, 8) . "...</p>";
} catch (Exception $e) {
    echo "<p class='error'>❌ Environment Variables Error: " . $e->getMessage() . "</p>";
}
echo "</div>";
...
try {
    ...
    if ($frontendConfig && isset($frontendConfig['token']) && isset($frontendConfig['apiKey'])) {
...
try {
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        'Authorization: ' . VideoSDKConfig::generateToken(),
