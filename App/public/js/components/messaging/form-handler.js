class FormHandler {
    ...
    initMessageForm() {
        ...
        this.submitButton = document.querySelector('#message-form button[type="submit"]');

        if (!this.messageForm || !this.messageInput) {
            this.messaging.debugUtils.log('⚠️ Message form elements not found - this is normal for pages without active chats');
            return;
        }
    ...
    setupFileUpload() {
        ...
        if (!this.messageInput) {
            this.messaging.debugUtils.error('❌ Cannot submit: message input not found');
            return;
        }
        ...
        try {
            ...
            tempMessageElement = this.messaging.messageHandler.createTemporaryMessage(content, chatType, chatId);
            ...
            // Send the message
            await this.messaging.messageHandler.sendMessage(chatType, chatId, originalContent);
    ...
    async handleFiles(files) {
        ...
        try {
            ...
            if (attachments.length > 0) {
                ...
                await this.messaging.messageHandler.sendRichMessage(chatType, chatId, messageData);
    ...
    async uploadFiles(files) {
        ...
        for (let file of files) {
            try {
                ...
                this.messaging.debugUtils.error('❌ Failed to upload file:', file.name, error);
            }
        }
        ...
        if (this.submitButton) {
            ...
            if (disabled) {
                ...
                this.submitButton.innerHTML = '<span class="animate-spin">⟳</span> Sending...';
            } else {
