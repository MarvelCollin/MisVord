class DebugUtils {
    ...
    getDebugInfo() {
        return {
            ...
            socketId: this.messaging.socketManager.socket ? this.messaging.socketManager.socket.id : 'none',
            errors: this.errors.slice(-5),
            connectionHistory: this.connectionHistory.slice(-10),
            typingUsers: Array.from(this.messaging.typingManager.typingUsers.entries()),
        ...
        if (typeof window !== 'undefined') {
            window.debugMessaging = {
                ...
                sendTestMessage: (content = 'Debug test message') => {
                    const chatId = this.messaging.activeChannel || this.messaging.activeChatRoom;
                    const chatType = this.messaging.chatType;
                    if (chatId && chatType) {
                        return this.messaging.messageHandler.sendMessage(chatType, chatId, content);
                    } else {
