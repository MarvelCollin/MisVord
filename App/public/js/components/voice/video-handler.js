document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            if (stream.stream) {
                console.log('[VideoHandler] getMediaStream: Checking stream.stream property:', {
                    ...
                    streamConstructor: stream.stream.constructor?.name,
                    streamKeys: Object.keys(stream.stream),
                    streamProto: Object.getPrototypeOf(stream.stream)
                });
                
                // Log all properties of the stream.stream object
                try {
                    console.log('[VideoHandler] getMediaStream: stream.stream properties:');
                    for (const [key, value] of Object.entries(stream.stream)) {
                        console.log(`  ${key}:`, {
                            type: typeof value,
                    ...
                    console.warn('[VideoHandler] getMediaStream: Error inspecting stream.stream:', e);
                }
                ...
                // Check if stream.stream has nested properties
                if (stream.stream.stream instanceof MediaStream) {
                    console.log('[VideoHandler] getMediaStream: Found nested MediaStream in .stream.stream');
                    return stream.stream.stream;
                }

                // Check for common MediaStream property names
                const possibleProps = ['mediaStream', 'localStream', 'remoteStream', 'videoStream', 'track', 'source'];
                for (const prop of possibleProps) {
                    if (stream.stream[prop] instanceof MediaStream) {
                        console.log(`[VideoHandler] getMediaStream: Found MediaStream in stream.stream.${prop}`);
                        return stream.stream[prop];
                    }
                }
            }
            ...
            // Last resort: Try to get MediaStream from VideoSDK participant directly
            if (window.videosdkMeeting && window.videosdkMeeting.localParticipant) {
                console.log('[VideoHandler] getMediaStream: Trying to get MediaStream from VideoSDK participant directly');
                const participant = window.videosdkMeeting.localParticipant;
                
                // Log complete participant structure for debugging
                console.log('[VideoHandler] getMediaStream: VideoSDK participant structure:', {
                    hasVideoTracks: !!participant.videoTracks,
                    videoTracksSize: participant.videoTracks?.size,
                    videoTracksType: typeof participant.videoTracks,
                    hasStreams: !!participant.streams,
                    streamsSize: participant.streams?.size,
                    streamsType: typeof participant.streams,
                    participantKeys: Object.keys(participant),
                    participantProps: Object.getOwnPropertyNames(participant)
                });
                
                // Check if participant has video tracks directly
                if (participant.videoTracks && participant.videoTracks.size > 0) {
                    console.log('[VideoHandler] getMediaStream: Found videoTracks on participant');
                    for (const [trackId, track] of participant.videoTracks) {
                        console.log(`[VideoHandler] getMediaStream: VideoTrack ${trackId}:`, {
                            isMediaStreamTrack: track instanceof MediaStreamTrack,
                        ...
                        if (track instanceof MediaStreamTrack) {
                            console.log(`[VideoHandler] getMediaStream: Creating MediaStream from track ${trackId}`);
                            return new MediaStream([track]);
                        }
                    }
                }
                
                // Check participant streams for video
                if (participant.streams && participant.streams.size > 0) {
                    console.log('[VideoHandler] getMediaStream: Checking participant streams directly');
                    for (const [streamId, streamObj] of participant.streams) {
                        console.log(`[VideoHandler] getMediaStream: Checking participant stream ${streamId}:`, {
                            streamObj: streamObj,
                            streamObjType: typeof streamObj,
                            streamObjConstructor: streamObj?.constructor?.name,
                            streamObjKeys: Object.keys(streamObj || {}),
                            hasStream: 'stream' in (streamObj || {}),
                            hasMediaStream: 'mediaStream' in (streamObj || {}),
                            hasTrack: 'track' in (streamObj || {})
                        });
                        
                        // Try to access MediaStream through different properties
                        const tryProps = ['stream', 'mediaStream', 'track', 'source'];
                        for (const prop of tryProps) {
                            if (streamObj[prop] instanceof MediaStream) {
                                console.log(`[VideoHandler] getMediaStream: Found MediaStream in participant.streams[${streamId}].${prop}`);
                                return streamObj[prop];
                            }
                            if (streamObj[prop] instanceof MediaStreamTrack) {
                                console.log(`[VideoHandler] getMediaStream: Found MediaStreamTrack in participant.streams[${streamId}].${prop}`);
                                return new MediaStream([streamObj[prop]]);
                            }
                        }
                    }
                }
                
                // Try getUserMedia as last resort for local participant
                if (stream && stream.kind === 'video') {
                    console.log('[VideoHandler] getMediaStream: Would attempt getUserMedia as fallback, but function must remain sync');
                    console.log('[VideoHandler] getMediaStream: Consider using testVideoStream() function for direct camera access');
                }
            }
    ...
    window.testDirectVideo = async function() {
        console.log('🧪 [Direct Video Test] Starting direct video test...');
        
        try {
            // Get direct MediaStream from camera
            const stream = await navigator.mediaDevices.getUserMedia({ 
                video: { width: 640, height: 480 }, 
                audio: false 
            });
            
            console.log('🧪 [Direct Video Test] Got MediaStream:', {
                id: stream.id,
            ...
            const fakeVideoSDKStream = {
                kind: 'video',
                stream: stream
            };
            
            console.log('🧪 [Direct Video Test] Testing with fake VideoSDK stream:', fakeVideoSDKStream);
            
            attachStream('local', fakeVideoSDKStream);
            
        } catch (error) {
            console.error('🧪 [Direct Video Test] Error:', error);
        }
    };
