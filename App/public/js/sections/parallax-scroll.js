...
function initParallaxScroll() {
  ...
  window.addEventListener("scroll", () => {
    ...
    if (scrollPosition > 50) {
      scrambleText.style.filter = `blur(${Math.min(
        scrollPercentage * 5,
        10
      )}px)`;
      scrambleText.style.opacity = Math.max(1 - scrollPercentage, 0);
      scrambleText.style.transform = `scale(${Math.max(
        1 - scrollPercentage * 0.3,
        0.7
      )}) translateY(${Math.min(scrollPercentage * 100, 100)}px)`;
    } else {
...
function revealFeatureCards() {
  const sectionTitle = document.querySelector(
    ".featured-cards-section .section-title"
  );
  const sectionSubtitle = document.querySelector(
    ".featured-cards-section .section-subtitle"
  );
...
function initScrollTransitions() {
  const sections = document.querySelectorAll(
    ".hero-section, .featured-cards-section"
  );
