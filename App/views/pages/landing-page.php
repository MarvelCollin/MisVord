    ...
    setTimeout(function() {
        ...
        if (heroTitle) {
            ...
            if (originalText) {
                ...
                const colorSchemes = [
                    { primary: '#5865F2', glow: 'rgba(88, 101, 242, 0.8)' }, 
                    { primary: '#EB459E', glow: 'rgba(235, 69, 158, 0.8)' },  
                    { primary: '#57F287', glow: 'rgba(87, 242, 135, 0.8)' },  
                    { primary: '#FEE75C', glow: 'rgba(254, 231, 92, 0.8)' }   
                ];
                ...
                const baseDelay = 80; 
                const charRevealBaseDelay = 60; 
                const charRevealRandomDelay = 40; 
                const scrambleIterations = 8; 
                const scrambleBaseSpeed = 30; 
                const scrambleRandomSpeed = 20; 
                        ...
                        setTimeout(() => {
                            ...
                            if (!span.classList.contains('revealed')) {
                                ...
                                if (i === scrambleIterations - 1) {
                                    ...
                                    const hue = 220 + (colorProgress * 40); 
                                    const lightness = 50 + (colorProgress * 20); 

                                    span.style.color = `hsl(${hue}, 80%, ${lightness}%)`;
                                    span.style.opacity = 0.3 + (progress * 0.7);

                                    const translateY = 30 - (progress * 35); 
                                    const rotateX = 20 - (progress * 30); 
                ...
                function initSuperGlitchEffects(spans, chars) {
                    ...
                    function glitchRandomCharacters() {
                        ...
                        const maxGlitchesAtOnce = Math.floor(Math.random() * 3) + 1; 
                        let glitchCount = 0;
                    ...
                    function superGlitchCharacter(span) {
                        ...
                        switch (glitchStyle) {
                            case 'intense':
                                glitchDuration = 800 + Math.random() * 600; 
                                frameInterval = 25 + Math.random() * 15; 
                                break;
                            case 'medium': 
                            ...
                            if (showRandomChar) {
                                ...
                                switch (glitchStyle) {
                                    ...
                                    case 'subtle':
                                        ...
                                        const tinyShiftY = subtlePulse - 0.4; 
                    ...
                    function scheduleNextGlitch() {
                        ...
                        if (rapidSequence) {
                            ...
                            let rapidCount = Math.floor(Math.random() * 3) + 2; 
                            let nextDelay = 200;
                    ...
                    spans.forEach(span => {
                        ...
                        span.addEventListener('mouseenter', () => {
                            ...
                            allSpans.forEach((neighborSpan, i) => {
                                ...
                                if (distance <= 5) { 
                                    const intensity = Math.pow(0.7, distance); 

                                    const rippleDelay = distance * 30; 
