...
function renderChannel($channel, $activeChannelId) {
    ...
    $baseClass = 'channel-item group flex items-center py-1 px-1 sm:px-2 rounded-md cursor-pointer mb-0.5 select-none transition-all duration-150';
    ...
    echo '    <i class="fas fa-' . $icon . ' text-[0.65rem] sm:text-[0.8rem] mr-1 sm:mr-2 ' . $iconClass . '"></i>';
    echo '    <span class="channel-name text-xs sm:text-sm truncate flex-1">' . htmlspecialchars($channel['name']) . '</span>';
    echo '  </div>';
    
    if ($type === 'voice') {
        $countClass = $isActive ? 'text-white/80' : 'text-gray-400 group-hover:text-gray-300';
        echo '</div>';
        echo '<div class="voice-participants ml-2 sm:ml-4 hidden rounded-md overflow-hidden" data-channel-id="' . $channel['id'] . '"></div>';
    } else {
        echo '</div>';
    }
}