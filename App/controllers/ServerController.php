...
class ServerController extends BaseController
{
    ...
    public function getServerLayoutHtml($serverId) {
        $this->requireAuth();

        try {
            error_log("[Server Layout HTML] Starting getServerLayoutHtml for server: " . $serverId);
            
            $server = $this->serverRepository->find($serverId);
            if (!$server) {
                error_log("[Server Layout HTML] Server not found: " . $serverId);
            ...
            $membership = $this->userServerMembershipRepository->findByUserAndServer($this->getCurrentUserId(), $serverId);
            if (!$membership) {
                error_log("[Server Layout HTML] User not member of server: " . $serverId);
            ...
            error_log("[Server Layout HTML] Generated HTML length: " . strlen($html));
            echo $html;
            exit;

        } catch (Exception $e) {
            error_log("[Server Layout HTML] Error: " . $e->getMessage());
            http_response_code(500);
            echo "Failed to load server layout";
            exit;
        }
    }
